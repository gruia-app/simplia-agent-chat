import { jsonValue, numberValue, recordValue, stringValue, } from "simplia-agent-chat/core/protocol";
import { event, providerFrom, threadScopedEntityId, threadScopedTurnId, } from "simplia-agent-chat/core/adapters/shared";
...
export const acv2DirectChatAdapter = {
    ...
    normalize(input, baseContext) {
        ...
        const assistantItemId = threadScopedEntityId(baseContext.threadId, undefined, `assistant:${nativeTurnId}`);
        ...
        const toolItems = rawToolCalls.flatMap((rawToolCall, index) => {
            ...
            const itemId = threadScopedEntityId(baseContext.threadId, undefined, `tool:${nativeTurnId}:${nativeToolId ?? "anonymous"}:${index}`);
            return [{
                    itemId,
