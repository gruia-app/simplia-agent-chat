...
function itemStatus(value, fallback) {
...
export const acv2PmAdapter = {
    ...
    normalize(input, baseContext) {
        ...
        if (eventKind === "tool_use" || eventKind === "tool_result") {
            ...
            events.push(event("item.upsert", context, suffix, {
                id: threadScopedEntityId(baseContext.threadId, nativeToolId, `tool:${runKey}:${toolUseId}`),
                threadId: context.threadId,
                turnId,
                kind: "tool",
                role: "tool",
        ...
        events.push(event("item.upsert", context, suffix, {
            id: threadScopedEntityId(baseContext.threadId, undefined, `system:${runKey}:${stableSuffix(cursor, nativeEvent)}`),
            threadId: context.threadId,
            turnId,
            kind: "system",
            role: "system",
