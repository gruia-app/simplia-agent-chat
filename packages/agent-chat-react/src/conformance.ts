import type { ConformanceCheck, ConformanceReport, ConformanceSeverity } from "simplia-agent-chat/core";
import { AGENT_CHAT_WORKSPACE_PANE_IDS, type AgentChatWorkspacePaneId } from "./AgentChatWorkspace.js";
...
function attributeValue(openingTag: string | undefined, name: string): string | undefined {
...
function hasAttribute(openingTag: string | undefined, name: string, value: string): boolean {
  return attributeValue(openingTag, name) === escapeHtml(value, true);
}

function paneIndex(html: string, id: AgentChatWorkspacePaneId): number {
  const tag = paneOpeningTag(html, id);
  return tag ? html.indexOf(tag) : -1;
}

function openingTagWithAttribute(html: string, name: string, value: string): string | undefined {
  const escaped = escapeHtml(value, true);
  for (const match of html.matchAll(/<[^>]+>/g)) {
    if (attributeValue(match[0], name) === escaped) return match[0];
  }
  return undefined;
}

function workspaceOpeningTag(html: string): string | undefined {
  return openingTagWithAttribute(html, "data-sac-workspace", "");
}

function paneOpeningTag(html: string, id: AgentChatWorkspacePaneId): string | undefined {
  return openingTagWithAttribute(html, "data-sac-pane", id);
}

function hasClassToken(openingTag: string | undefined, token: string): boolean {
  const classes = attributeValue(openingTag, "class");
  return classes?.split(/\s+/).includes(token) ?? false;
}

function openingTagWithClass(html: string, token: string): string | undefined {
  for (const match of html.matchAll(/<[^>]+>/g)) {
    if (hasClassToken(match[0], token)) return match[0];
  }
  return undefined;
}

function elementSegment(html: string, openingTag: string | undefined): string | undefined {
  if (!openingTag) return undefined;
  const start = html.indexOf(openingTag);
  const tagName = openingTag.match(/^<([a-z][\w-]*)\b/i)?.[1];
  if (start < 0 || !tagName) return undefined;
  ...
  for (const match of html.slice(start).matchAll(matcher)) {
    ...
    if (depth === 0) return html.slice(start, start + (match.index ?? 0) + tag.length);
  }
  return openingTag;
}

function paneSegment(html: string, id: AgentChatWorkspacePaneId): string | undefined {
  return elementSegment(html, paneOpeningTag(html, id));
}

function leaksCanary(html: string, canary: string): boolean {
  if (!canary.trim()) return true;
...
const PANE_ORDER: readonly AgentChatWorkspacePaneId[] = [
...
export function runWorkspaceMarkupConformance(input: WorkspaceMarkupConformanceInput): ConformanceReport {
  ...
  checks.push(check(
    "workspace.region",
    "Workspace root exposes a labeled region landmark",
    expected.ariaLabel.trim().length > 0
      && hasAttribute(rootTag, "role", "region")
      && hasAttribute(rootTag, "aria-label", expected.ariaLabel),
    DETAIL.regionPresent,
    DETAIL.regionMissing,
  ));

  checks.push(check(
    "workspace.layout",
    "Workspace layout attribute matches home or work presence",
    hasAttribute(rootTag, "data-sac-workspace-layout", expected.layout),
    DETAIL.layoutPresent,
    DETAIL.layoutMissing,
  ));

  checks.push(check(
    "workspace.history-label",
    "History pane keeps the application-provided landmark label",
    normalizedVisiblePanes.includes(AGENT_CHAT_WORKSPACE_PANE_IDS.history)
      ? expected.historyLabel.trim().length > 0
        && hasAttribute(
          paneOpeningTag(html, AGENT_CHAT_WORKSPACE_PANE_IDS.history),
          "aria-label",
          expected.historyLabel,
        )
      : true,
    DETAIL.historyLabelPresent,
    DETAIL.historyLabelMissing,
  ));

  if (normalizedVisiblePanes.includes(AGENT_CHAT_WORKSPACE_PANE_IDS.workQueue)) {
    checks.push(check(
      "workspace.queue-label",
      "Work queue pane keeps the application-provided landmark label",
      typeof expected.workQueueLabel === "string"
        && expected.workQueueLabel.trim().length > 0
        && hasAttribute(
          paneOpeningTag(html, AGENT_CHAT_WORKSPACE_PANE_IDS.workQueue),
          "aria-label",
          expected.workQueueLabel,
        ),
      DETAIL.queueLabelPresent,
      DETAIL.queueLabelMissing,
    ));
  }

  if (normalizedVisiblePanes.includes(AGENT_CHAT_WORKSPACE_PANE_IDS.workbench)) {
    checks.push(check(
      "workspace.workbench-label",
      "Workbench pane keeps the application-provided landmark label",
      typeof expected.workbenchLabel === "string"
        && expected.workbenchLabel.trim().length > 0
        && hasAttribute(
          paneOpeningTag(html, AGENT_CHAT_WORKSPACE_PANE_IDS.workbench),
          "aria-label",
          expected.workbenchLabel,
        ),
      DETAIL.workbenchLabelPresent,
      DETAIL.workbenchLabelMissing,
    ));
  }
  ...
  checks.push(check(
    "workspace.pane-order",
    "Visible panes stay in documented DOM order",
    orderPassed,
    DETAIL.paneOrder,
    DETAIL.paneOrderBroken,
  ));

  const hiddenPanes = PANE_ORDER.filter((id) => !normalizedVisiblePanes.includes(id));
  checks.push(check(
    "workspace.hidden-panes",
    "Hidden or omitted panes are unmounted",
    hiddenPanes.every((id) => paneIndex(html, id) === -1),
    DETAIL.hiddenAbsent,
    DETAIL.hiddenPresent,
  ));

  if (expected.textOnly) {
    checks.push(check(
      "workspace.no-library-nav",
      "Text-only workspace fixtures do not invent navigation buttons",
      !/<button\b/i.test(html),
      DETAIL.noLibraryButtons,
      DETAIL.libraryButtons,
    ));
  }

  if (expected.nestedShell) {
    checks.push(check(
      "workspace.nested-shell",
      "Conversation pane includes the nested shell marker",
      hasClassToken(
        openingTagWithClass(
          paneSegment(html, AGENT_CHAT_WORKSPACE_PANE_IDS.conversation) ?? "",
          "sac-shell",
        ),
        "sac-shell",
      ),
      DETAIL.nestedShellPresent,
      DETAIL.nestedShellMissing,
    ));
  }
  ...
  if (canary) {
    checks.push(check(
      "workspace.canary-isolation",
      "Workspace markup does not leak the payload canary",
      !leaksCanary(html, canary),
...
export function runSurfaceHostMarkupConformance(input: SurfaceHostMarkupConformanceInput): ConformanceReport {
  const html = input.html;
  const expected = input.expected;
  const checks: ConformanceCheck[] = [];
  ...
  checks.push(check(
    "surface-host.markup",
    "Surface host renders trusted or fail-closed fallback markup",
    expected.fallback ? fallback : trusted || fallback,
    expected.fallback ? DETAIL.surfaceFallback : DETAIL.surfaceReady,
    DETAIL.surfaceMissing,
  ));

  if (expected.kind) {
    checks.push(check(
      "surface-host.kind",
      "Surface host markup includes the escaped surface kind",
      expected.kind.trim().length > 0
        && (
          surfaceHtml.includes(escapeHtml(expected.kind, false))
          || surfaceHtml.includes(escapeHtml(expected.kind, true))
        ),
      DETAIL.surfaceKindPresent,
      DETAIL.surfaceKindMissing,
    ));
  }
  ...
  if (canary) {
    checks.push(check(
      "surface-host.canary-isolation",
      "Surface host markup does not leak the payload canary",
      !leaksCanary(html, canary),
      DETAIL.canaryIsolated,
      DETAIL.canaryLeaked,
    ));
