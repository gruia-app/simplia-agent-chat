...
function InteractionCard({ interaction, onResolve, }) {
    ...
    return (_jsxs("section", { className: `sac-interaction sac-interaction-${interaction.kind}`, "aria-labelledby": titleId, ...(resolution.inFlight ? { "aria-busy": true } : {}), children: [_jsxs("div", { className: "sac-interaction-copy", children: [_jsx("span", { className: "sac-eyebrow", children: interaction.kind === "approval" ? "APPROVAL_REQUIRED" : "INPUT_REQUIRED" }), _jsx("h3", { id: titleId, children: interaction.title }), interaction.description ? _jsx("p", { children: interaction.description }) : null, requiresConfirmation ? (_jsx("p", { className: "sac-interaction-confirm-hint", children: "Selecting a decision requires Confirm before it is submitted." })) : null] }), decisions.length > 0 ? (confirmation !== undefined ? (_jsxs("div", { className: "sac-interaction-actions sac-interaction-confirm", role: "group", "aria-label": `Confirm ${confirmation} for ${interaction.title}`, children: [_jsxs("p", { className: "sac-interaction-confirm-choice", children: ["Confirm decision: ", _jsx("strong", { children: confirmation })] }), _jsxs("button", { className: `sac-button${isAffirmativeDecision(confirmation) ? " sac-button-primary" : ""}`, type: "button", disabled: resolution.inFlight, "aria-label": `Confirm ${confirmation} for ${interaction.title}`, ref: confirmButtonRef, onClick: () => submitResolution({ decision: confirmation }), children: ["Confirm ", confirmation] }), _jsx("button", { className: "sac-button", type: "button", disabled: resolution.inFlight, "aria-label": `Back to decisions for ${interaction.title}`, onClick: () => {
                            const next = cancelApprovalConfirmation(resolutionRef.current);
                            resolutionRef.current = next;
                            setResolution(next);
                        }, children: "Back" })] })) : (_jsx("div", { className: "sac-interaction-actions", role: "group", "aria-label": `Respond to ${interaction.title}`, children: decisions.map((decision) => (_jsx("button", { className: `sac-button${isAffirmativeDecision(decision) ? " sac-button-primary" : ""}`, type: "button", disabled: resolution.inFlight, onClick: () => chooseDecision(decision), children: decision }, decision))) }))) : (_jsxs("form", { className: "sac-answer-form", onSubmit: submitAnswer, children: [_jsx("label", { htmlFor: `${titleId}-answer`, children: `Answer for ${interaction.title}` }), _jsxs("div", { children: [_jsx("input", { id: `${titleId}-answer`, value: answer, onChange: (event) => setAnswer(event.target.value), autoComplete: "off", disabled: resolution.inFlight }), _jsx("button", { className: "sac-button sac-button-primary", type: "submit", disabled: resolution.inFlight || !answer.trim(), children: "Submit" })] })] })), resolutionError ? _jsx("p", { className: "sac-interaction-error", role: "alert", children: resolutionError }) : null] }));
}
export function PendingInteractions({ interactions, onResolve }) {
    const pending = interactions.filter((interaction) => interaction.status === "pending");
    if (pending.length === 0)
        return null;
    return (_jsx("div", { className: "sac-interactions", "aria-label": "Pending agent interactions", children: pending.map((interaction) => (_jsx(InteractionCard, { interaction: interaction, onResolve: onResolve }, interaction.id))) }));
}
