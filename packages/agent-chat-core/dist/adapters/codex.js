...
function normalizeItem(itemInput, context) {
    const item = recordValue(itemInput);
    const nativeType = stringValue(item.type) ?? "unknown";
    const nativeItemId = stringValue(item.id);
    const id = threadScopedEntityId(context.threadId, nativeItemId, `codex-item:${stableSuffix(nativeType, context.sequence)}`);
...
export const codexAppServerAdapter = {
    ...
    normalize(input, context) {
        ...
        if (deltaConfig) {
            ...
            const itemId = threadScopedEntityId(context.threadId, nativeItemId, `${deltaConfig.kind}:${nativeTurnId ?? "turn"}`);
