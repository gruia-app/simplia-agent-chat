import { jsonValue, recordValue, stringValue, } from "../protocol.js";
    ...
    normalize(input, context) {
        ...
        if (mode === "messages") {
            ...
            if (content) {
                events.push(event("item.delta", eventContext, suffix, {
                    itemId: threadScopedEntityId(context.threadId, nativeMessageId, `assistant:${context.turnId ?? "turn"}`),
                    delta: content,
            ...
            for (const [index, toolCallInput] of toolCalls.entries()) {
                ...
                events.push(event("item.upsert", eventContext, `${suffix}:tool:${index}`, {
                    id: threadScopedEntityId(context.threadId, nativeToolId, `tool:${context.turnId ?? "turn"}:${index}`),
            ...
            for (const [nodeName, nodeInput] of Object.entries(updates)) {
                ...
                for (const [index, messageInput] of messages.entries()) {
                    ...
                    events.push(event("item.upsert", eventContext, `${suffix}:${nodeName}:${index}`, {
                        id: threadScopedEntityId(context.threadId, nativeMessageId, `langchain:${context.turnId ?? "turn"}:${nodeName}:${index}`),
                        threadId: context.threadId,
                        turnId,
        ...
        if (mode === "custom") {
            events.push(event("surface.upsert", eventContext, suffix, {
                id: threadScopedEntityId(context.threadId, undefined, `langchain.custom:${context.turnId ?? "turn"}:${stableSuffix(input.name, context.sequence)}`),
                threadId: context.threadId,
                turnId,
        ...
        if (eventName.endsWith("_stream")) {
            ...
            const content = messageParts(Object.keys(nestedChunk).length > 0 ? nestedChunk.content : chunk.chunk ?? chunk.content) || stringValue(nestedChunk.text) || stringValue(chunk.text) || "";
            if (content) {
                events.push(event("item.delta", eventContext, suffix, {
                    itemId: threadScopedEntityId(context.threadId, undefined, `assistant:${context.turnId ?? "turn"}`),
                    delta: content,
                }, provider));
            }
        }
