...
export function eventBase(type, context, suffix) {
...
const CHAT_EVENT_TYPE_SET = new Set(CHAT_EVENT_TYPES);
...
function hasMatchingThread(entityThreadId, envelopeThreadId) {
    return entityThreadId === envelopeThreadId;
}
function hasMatchingTurn(entityTurnId, envelopeTurnId) {
...
function isPendingInteraction(value) {
    if (!isPlainObject(value))
        return false;
    return Boolean(stringValue(value.id)
        && stringValue(value.threadId)
...
function isThreadSnapshot(value) {
    if (!isPlainObject(value))
        return false;
    if (!isChatThread(value.thread))
        return false;
    if (!Array.isArray(value.turns) || !value.turns.every(isChatTurn))
        return false;
    if (!Array.isArray(value.items) || !value.items.every(isChatItem))
        return false;
    if (!Array.isArray(value.surfaces) || !value.surfaces.every(isSurfaceBlock))
        return false;
    if (!Array.isArray(value.interactions) || !value.interactions.every(isPendingInteraction))
        return false;
    return true;
}
function payloadAgreesWithEnvelope(type, envelope, payload) {
...
export function validateChatEvent(input) {
    ...
    if (typeof input.type !== "string" || !input.type.trim())
        return fail("invalid_event");
    if (!CHAT_EVENT_TYPE_SET.has(input.type))
    ...
    if (input.turnId !== undefined && !stringValue(input.turnId))
        return fail("invalid_event");
    if (input.payload === undefined)
        return fail("invalid_event");
    if (input.stream !== undefined) {
        if (!isPlainObject(input.stream))
            return fail("invalid_event");
    ...
    if (input.provider !== undefined) {
        if (!isPlainObject(input.provider) || !stringValue(input.provider.provider))
            return fail("invalid_event");
    }
    if (!payloadAgreesWithEnvelope(input.type, input, input.payload))
        return fail("invalid_event");
