import { validateChatEvent, } from "./protocol.js";
...
function withStreamSequence(state, event) {
    if (!event.stream)
        return state;
    return {
        ...state,
        streamSequences: { ...state.streamSequences, [event.stream.id]: event.stream.sequence },
        resyncRequests: Object.fromEntries(Object.entries(state.resyncRequests).filter(([streamId]) => streamId !== event.stream?.id)),
    };
}
function patchSurface(surface, patch) {
...
function fallbackTurnId(threadId) {
...
function snapshotHasForeignCollision(state, snapshot) {
...
function eventConflictsWithForeignThread(state, event) {
    switch (event.type) {
...
function applyEvent(state, event) {
    switch (event.type) {
        case "thread.snapshot": {
            ...
            const threadId = snapshot.thread.id;
            const turns = Object.fromEntries(Object.entries(state.turns).filter(([, turn]) => turn.threadId !== threadId));
            const items = Object.fromEntries(Object.entries(state.items).filter(([, item]) => item.threadId !== threadId));
            const surfaces = Object.fromEntries(Object.entries(state.surfaces).filter(([, surface]) => surface.threadId !== threadId));
            const interactions = Object.fromEntries(Object.entries(state.interactions).filter(([, interaction]) => interaction.threadId !== threadId));
            for (const turn of snapshot.turns)
                turns[turn.id] = turn;
            for (const item of snapshot.items)
                items[item.id] = item;
        ...
        case "item.upsert": {
            ...
            const nextItem = previousItem && item.kind === "tool" && previousItem.kind === "tool"
                ? {
                    ...previousItem,
                    ...item,
...
export function reduceChatEvent(state, event) {
    ...
    if (!validated.ok)
        return { state, applied: false, reason: validated.reason };
    ...
    if (state.seenEventIds.includes(nextEvent.id))
...
export function replayChatEvents(events, initial = createInitialChatState()) {
    let state = initial;
    for (const event of events)
        state = reduceChatEvent(state, event).state;
    return state;
}
export function selectThreadTurns(state, threadId) {
    return Object.values(state.turns)
        .filter((turn) => turn.threadId === threadId)
        .sort((a, b) => (a.startedAt ?? a.id).localeCompare(b.startedAt ?? b.id));
}
export function selectTurnItems(state, turnId) {
    const turn = state.turns[turnId];
    if (!turn)
        return [];
...
export function selectActiveTurn(state, threadId) {
...
function resolveWaitingKind(turn, pending) {
    if (pending)
...
function threadRunState(phase, parts = {}) {
    return {
        phase,
...
export function selectThreadRunState(state, threadId) {
