...
const LIMIT_KINDS = new Set<ChatLimitKind>([
  "quota_exceeded",
  "plan_required",
  "rate_limited",
  "custom",
 ...
 */
export function normalizeChatLimitNotice(value: unknown): ChatLimitNotice | undefined {
  if (!isPlainObject(value)) return undefined;
  const kind = typeof value.kind === "string" && LIMIT_KINDS.has(value.kind as ChatLimitKind)
    ? (value.kind as ChatLimitKind)
    : undefined;
  const message = boundedString(value.message, MAX_LIMIT_MESSAGE);
  if (!kind || !message) return undefined;
  if (value.actionLabel !== undefined && !boundedString(value.actionLabel, MAX_LIMIT_LABEL)) return undefined;
  if (value.actionId !== undefined && !boundedString(value.actionId, MAX_LIMIT_ACTION_ID)) return undefined;
  ...
  const notice: ChatLimitNotice = { kind, message };
  if (actionLabel) notice.actionLabel = actionLabel;
  if (actionId) notice.actionId = actionId;
  if (value.blocking === false) notice.blocking = false;
